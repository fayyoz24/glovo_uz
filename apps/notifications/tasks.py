...
def send_otp_sms(self, phone: str, code: str, lang: str = "uz") -> None:
    """
    Dedicated OTP task – always SMS only, always transactional.
    Called from accounts app OTP service.
    """
    from apps.notifications.services import NotificationService
    from apps.notifications.constants import NotificationEvent, NotificationType

    try:
        NotificationService.send_event(
            event=NotificationEvent.OTP_SENT,
            recipient_id=0,  # No user yet (pre-registration)
            context={"code": code},
            phone=phone,
            lang=lang,
        ...
        logger.exception("send_otp_sms failed for phone %s", phone)
        raise self.retry(exc=exc)


# ================================================================== #
#  Maintenance tasks – run by Celery Beat                             #
# ================================================================== #
